...
function SimPill() {
  return (
    <span className="ml-2 px-1.5 py-0.5 rounded font-mono text-[10px] uppercase tracking-[0.08em] border border-[#ff9c0044] text-[#ff9c00] bg-[#ff9c000d]">
      simulated
    </span>
  );
}
...
function WalletSection() {
  ...
  return (
    <div className="p-4 rounded border border-[var(--border)] bg-[var(--bg-panel)] flex flex-col gap-3">
      <div className="flex items-center justify-between">
        <span className="font-mono text-[10px] text-[var(--text-faint)] uppercase tracking-[0.08em]">Wallet</span>
        <span className="font-mono text-[10px] text-[var(--text-faint)] uppercase tracking-[0.08em]">0G Galileo testnet</span>
      </div>
      <button
...
export default function ConnectPage() {
    ...
    return (
      <main className="min-h-screen flex flex-col items-center justify-center px-6">
        <div className="max-w-sm w-full flex flex-col gap-5">
          <Link href="/" className="font-mono text-[10px] text-[var(--text-faint)] uppercase tracking-[0.08em] hover:text-[var(--text-muted)] transition-colors">
            ← Back
          </Link>
          <div className="p-5 rounded border border-[var(--border)] bg-[var(--bg-panel)] flex flex-col gap-4">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[14px] text-[var(--text)]">Signed in as <b className="font-medium text-[var(--green)]">{current}</b></span>
            </div>
            <div className="flex gap-2">
              <Link href="/marketplace" className="flex-1 py-2 bg-[var(--green)] text-black font-mono text-[10px] font-bold uppercase tracking-[0.08em] rounded text-center hover:opacity-90 transition-opacity">
                Go to App →
              </Link>
              <button onClick={handleSignOut} className="px-3 py-2 border border-[var(--border)] font-mono text-[10px] text-[var(--text-muted)] uppercase tracking-[0.08em] rounded hover:border-[var(--border-soft)] transition-colors">
                Sign Out
              </button>
            </div>
          </div>
  ...
  return (
    <main className="min-h-screen flex flex-col items-center justify-center px-6">
      <div className="max-w-sm w-full flex flex-col gap-5">
        <div>
          <Link href="/" className="font-mono text-[10px] text-[var(--text-faint)] uppercase tracking-[0.08em] hover:text-[var(--text-muted)] transition-colors mb-3 block">
            ← Back
          </Link>
          <h2 className="text-[32px] leading-tight text-[var(--text)]" style={{ fontFamily: "var(--font-serif)", fontStyle: "italic" }}>
            Connect
          </h2>
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="font-mono text-[10px] text-[var(--text-faint)] uppercase tracking-[0.08em]">Orchestrator</span>
          <div className="flex-1 h-px bg-[var(--border)]" />
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-3">
          <div className="flex flex-col gap-1.5">
            <label className="font-mono text-[10px] text-[var(--text-muted)] uppercase tracking-[0.08em]">Username</label>
            <input
              type="text"
              value={username}
          ...
          <div className="flex flex-col gap-1.5">
            <label className="font-mono text-[10px] text-[var(--text-muted)] uppercase tracking-[0.08em]">Password</label>
            <input
              type="password"
              value={password}
          ...
          {tab === "register" && (
            <div className="flex flex-col gap-1.5">
              <label className="font-mono text-[10px] text-[var(--text-muted)] uppercase tracking-[0.08em]">Confirm Password</label>
              <input
                type="password"
                value={confirm}
